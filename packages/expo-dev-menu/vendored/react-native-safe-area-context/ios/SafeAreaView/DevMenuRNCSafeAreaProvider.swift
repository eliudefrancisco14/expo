...
class SafeAreaProvider: UIView {
    ...
    var onInsetsChange: (([String: Any]) -> Void) = {_ in }
    ...
    func invalidateSafeAreaInsets() {
        ...
        if _initialInsetsSent && DevMenuUIEdgeInsetsEqualToEdgeInsetsWithThreshold(safeAreaInsets, _currentSafeAreaInsets, 1.0 / RCTScreenScale()) && frame.equalTo(_currentFrame) {
            return
        }
